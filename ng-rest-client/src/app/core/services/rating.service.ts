...
export class RatingService {
  ...
  fetchRatingsForCatalogItem(catalogItemId: string) {
    return this.http.get<any>(`${environment.apiUrl}/rating/api/${catalogItemId}`);
  }

  addRatingToCatalogItem({catalogItemId, rating}: AddRatingEvent) {
    return this.http.post<Rating>(`${environment.apiUrl}/rating/api/${catalogItemId}`, {catalogItemId, rating});
  }
}
