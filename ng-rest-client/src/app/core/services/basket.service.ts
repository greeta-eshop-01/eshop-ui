...
export class BasketService {
  ...
  getBasket(customerId: string): Observable<CustomerBasket> {
    return this.http.get<CustomerBasket>(`${environment.apiUrl}/basket/api/customer/${customerId}`).pipe(
  ...
  updateQuantities(quantities: number[]): Observable<CustomerBasket> {
    ...
    return this.http.put<CustomerBasket>(`${environment.apiUrl}/flux-gateway/api/v1/basket/items`, basketItemUpdates).pipe(
      tap(basket => this.customerBasket = basket)
    );
  }
  ...
  private updateBasket(basketItems: CustomerBasket): Observable<CustomerBasket> {
    return this.http.post<CustomerBasket>(`${environment.apiUrl}/flux-gateway/api/v1/basket`, basketItems).pipe(
      tap(basket => this.customerBasket = basket)
    );
  }
